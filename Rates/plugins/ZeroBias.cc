  ...
  _triggerBits    (consumes<edm::TriggerResults>                    (iConfig.getParameter<edm::InputTag>("triggerResultsLabel")))
{
  ...
  this -> _triggerNamesTree -> Branch("higgsGroups",&higgsGroups);
  this -> _triggerNamesTree -> Branch("prescale",&prescale);
  ...
  for (const edm::ParameterSet& parameterSet : HLTList) {
    ...
    pSet.HiggsGroup = parameterSet.getParameter<std::string>("HiggsGroup");
    higgsGroups = parameterSet.getParameter<std::string>("HiggsGroup");
    prescale = parameterSet.getParameter<int>("prescale");
...
void ZeroBias::beginJob()
{
  ...
  //Branches
  this -> _tree -> Branch("EventNumber",  &_indexevents,"EventNumber/I");
  this -> _tree -> Branch("RunNumber",  &_runNumber,"RunNumber/I");
  this -> _tree -> Branch("lumi",  &_lumi,"lumi/I");
  // this -> _tree -> Branch("lumi",  &_lumi, "lumi/L");
